...
int main(int argc, char **argv) {
  ...
  ros::Publisher vel_pub =
      n.advertise<geometry_msgs::Vector3Stamped>("velocity", 1000);
  ros::Publisher radar_pub =
      n.advertise<sensor_msgs::PointCloud>("cfar_detections", 1000);
  ros::Rate loop_rate(10);

  // Initialize radar sensor interface.
  SensorConfig cfg;
  cfg.set("path_cfg_file",
          ros::package::getPath("mav_sensors_demo") +
              "/cfg/radar/xwr18xx_AOP_profile_best_velocity_resolution.cfg");
  ...
  while (ros::ok()) {
    ...
    if (rio::leastSquares(std::get<Radar>(measurement), &velocity)) {
      ...
      msg_velocity.header.stamp =
          rio::toRosTime(std::get<Radar>(measurement).unix_stamp_ns);
    ...
    msg.channels[0].values.resize(
        std::get<Radar>(measurement).cfar_detections.size());
    ...
    msg.channels[2].values.resize(
        std::get<Radar>(measurement).cfar_detections.size());
    for (size_t i = 0; i < std::get<Radar>(measurement).cfar_detections.size();
         i++) {
      ...
      msg.channels[1].values[i] =
          std::get<Radar>(measurement).cfar_detections[i].snr;
      msg.channels[2].values[i] =
          std::get<Radar>(measurement).cfar_detections[i].noise;
    }
