...
if __name__ == '__main__':
    # Open rosbag
    bag = rosbag.Bag('/home/brik/Desktop/paper_analysis/baro_residuals_fair.bag')

    # Read topics rio/DopplerResidual
    residuals = []
    for topic, msg, t in bag.read_messages(topics=['/rio/baro_residual']):
        residuals.append(msg.residual)

    sigma = 0.2
    residuals = np.array(residuals) / sigma
    ...
    #ax.set_xscale('log')
    counts, bins, bars = ax.hist(residuals, density=True, bins=50, log=False, alpha=0.5, label=r'$\textrm{Baro}$')
    ax.set_xlabel(r'$\displaystyle r_{B^{i}} / \sigma_B$')
    ax.set_ylabel(r'\textrm{Probability Density}')
    ax.grid()
    # Save figure
    x = np.logspace(np.log10(0.1),np.log10(max(residuals)), 300)
    x = np.linspace(-max(np.abs(residuals)), max(np.abs(residuals)), 300)
    c_fair = 1.3998
    c_huber = 1.345
    c_cauchy = 2.3849
    c_geman_mcclure = 3.0
    ...
    ax.plot(x, stats.norm.pdf(x), label='$\displaystyle \mathcal{N}(0,1)$')
    ax.set_ylim(min(counts[np.where(counts > 0)]), max(counts))
    ax_right.set_ylabel(r'$\displaystyle \rho \left( r_{B^{i}} / \sigma_B \right)$')
    ...
    ax_right.legend(loc='center right',prop={'size': 8}, bbox_to_anchor=(1.01, 0.7))
    ax.legend(loc='center left',prop={'size': 8}, bbox_to_anchor=(0.0, 0.7))
    #plt.show()
    plt.tight_layout()
    plt.savefig(f'/home/brik/Desktop/paper_analysis/residuals_baro.pdf', bbox_inches='tight')
